...
export class McpServerModal extends Modal {
  ...
  private existingServer: McpServer | null;
  ...
  onOpen() {
    ...
    this.modalEl.addClass('oa-mcp-modal');

    const { contentEl } = this;

    new Setting(contentEl)
      .setName('Server name')
      .setDesc('Unique identifier (letters, numbers, dots, hyphens, underscores)')
      .addText((text) => {
    ...
    this.typeFieldsEl = contentEl.createDiv({ cls: 'oa-mcp-type-fields' });
    this.renderTypeFields();
    ...
    const buttonContainer = contentEl.createDiv({ cls: 'oa-mcp-buttons' });
  ...
  private renderStdioFields() {
    ...
    cmdTextarea.placeholder = 'npx -y @modelcontextprotocol/server-filesystem /path';
    cmdTextarea.rows = 2;
