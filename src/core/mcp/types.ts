 ...
 *
 * Types for configuring and managing MCP servers that extend the agent's capabilities.
...
/** Extended server configuration with obsidian-agent metadata. */
...
/** Extended config file with obsidian-agent metadata. */
export interface McpConfigFileWithMeta extends McpConfigFile {
  _obsidianAgent?: {
    /** Per-server obsidian-agent-specific settings. */
    servers: Record<
      string,
      {
        enabled?: boolean;
...
export const DEFAULT_MCP_SERVER: Omit<McpServer, 'name' | 'config'> = {
  enabled: true,
  contextSaving: false,
};
